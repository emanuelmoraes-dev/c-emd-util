...
#define LIST_FIND_BY_INDEX(varname, index) \
    varname##_linked_list__ != NULL ? linked_list_find_by_index(varname##_linked_list__, index) : \
    array_list_find_by_index(varname##_array_list__, index)
 ...
 * @return a referência do valor buscado. NULL caso não seja encontrado
 */
#define LIST_REF_FIND_BY_INDEX \
    varname##_linked_list__ != NULL ? linked_list_find_by_index : array_list_find_by_index
